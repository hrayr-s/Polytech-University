...
urlpatterns = [
    ...
    url(r'^ֆակուլտետ/', include([
        url(r'^ավելացնել.html$', Fakultet.Add.as_view(), name="add-faculty"),
        url(r'^(?P<pk>[0-9]+)/խմբագրել.html', Fakultet.Edit.as_view(), name="edit-faculty"),
        url(r'^(?:/ցուցակ)?(?:/page-(?P<page>[0-9]+))?.html', Fakultet.List.as_view(), name="list-faculties"),
    ])),
    url(r'^խումբ/', include([
